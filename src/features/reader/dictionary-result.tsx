...
// for colours sized to the reading theme; bind them to the current text colour so
...
interface Props {
  result: LookupResult;
  /** Mines an entry to Anki. Absent hides the per-entry Anki button. */
...
/**
 * The body of a lookup: term entries, then kanji cards. Carries no placement of
 * its own, so it serves both the reader's floating popup and the standalone
 * lookup panel on the Words page.
 */
export function DictionaryResult({ result, onMine, onMineKanji, onSpeak }: Props) {
